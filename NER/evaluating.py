...
class Metrics(object):
    ...
    def cal_precision(self):
        ...
        for tag in self.tagset:
            number = self.correct_tags_number.get(tag, 0)
            if number:
                precision_scores[tag] = number / self.predict_tags_counter[tag]
            else:
    ...
    def cal_recall(self):
        ...
        for tag in self.tagset:
            number = self.correct_tags_number.get(tag, 0)
            if number:
                recall_scores[tag] = number / self.golden_tags_counter[tag]
            else:
                recall_scores[tag] = 0
